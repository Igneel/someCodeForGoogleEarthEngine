...
//var boundaries = ee.FeatureCollection('USDOS/LSIB/2013')

//boundaries=boundaries.filter(ee.Filter.eq('cc','RS'))
//Map.addLayer(boundaries, {}, 'default display');

//print(boundaries);

var germany=ee.FeatureCollection(ee.Geometry.Polygon([
  [[37.7485733,55.9898274], [37.8241043,56.0692478], [37.889679,55.997892], [37.8114014,55.9456343]]
  ])); // здесь нужно добавить координат, чтобы описать границы города
...
Map.setCenter(37.8405838,56.0001958, 11);
...
Export.table.toDrive(cities, 'greencities_export','jsData','cities', { fileFormat: 'GeoJSON' });
...
Export.image.toDrive(ndviRGB, 'germany_ndvi','jsData','coloredShapeOfGermany', {
  scale: 30,
...
function getLandsatByYear(year) {
  return landsats
    ...
    .filter(ee.Filter.gt('to', year)).first();
}
